...
//Export
$select = 'SELECT subscribers.name, subscribers.email, subscribers.join_date, subscribers.timestamp, subscribers.list, subscribers.ip, subscribers.country, subscribers.referrer, subscribers.method, subscribers.added_via, lists.name as list_name  
			FROM subscribers 
			LEFT JOIN lists
...
if($export)
{
	while($row = mysqli_fetch_array($export))
    {
		...
		$data .= $name.','.$email.','.$list_name.',"'.$join_date.'","'.$last_activity.'","'.$added_via.'","'.$optin_method.'","'.$ip.'","'.$signedup_country.'","'.$signedup_country_code.'","'.$referrer.'"'."\n";
    } 
    $data = substr($data, 0, -1);
    
    $first_line = '"'._('Name').'","'._('Email').'","'._('List').'","'._('Joined').'","'._('Last activity').'","'._('Added via').'","'._('Opt-in method').'","'._('IP address').'","'._('Country').'","'._('Country code').'","'._('Signed up from').'"'."\n";
